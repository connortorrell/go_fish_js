class GameView {
  ...
  gameResults() {
    if(this.game().results().length !== 0){
      return this.game().results().map(result => result.turnPlayerName() == this.game().turnPlayer().name() ? `<li><strong>${result.message()}</strong></li>` : `<li>${result.message()}</li>`).reverse().join('')
    } else {
